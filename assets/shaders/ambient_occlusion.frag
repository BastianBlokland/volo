...
bind_global_img(0) uniform sampler2D u_texGeoNormal;
...
void main() {
  const f32v4 geoNormal   = texture(u_texGeoNormal, in_texcoord);
  const f32v3 worldNormal = geometry_decode_normal(geoNormal);

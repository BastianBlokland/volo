 ...
 * halfway vector.
 */
f32 light_distribution_ggx(const f32v3 normal, const f32v3 halfDir, const f32 roughness) {
...
f32 light_geometry_schlick_ggx(const f32 normDotView, const f32 roughness) {
...
f32 light_geometry_smith(
  ...
  const f32 ggx2         = light_geometry_schlick_ggx(normDotView, roughness);
  const f32 ggx1         = light_geometry_schlick_ggx(normDotLight, roughness);
  return ggx1 * ggx2;
}
 ...
 */
f32v3 light_fresnel_schlick(const f32 cosTheta, const f32v3 reflectance) {
...
f32v3 light_surf_reflectance(const Surface surf) {
  /**
...
f32v3 light_resolve_directional(
    const f32v3 radiance, const f32v3 dir, const f32v3 viewDir, const Surface surf) {

  const f32v3 halfDir     = normalize(viewDir - dir);
  const f32v3 reflectance = light_surf_reflectance(surf);

  // Cook-Torrance BRDF.
  const f32   normDistFrac = light_distribution_ggx(surf.normal, halfDir, surf.roughness);
  const f32   geoFrac      = light_geometry_smith(surf.normal, viewDir, -dir, surf.roughness);
  const f32v3 fresnelFrac  = light_fresnel_schlick(max(dot(halfDir, viewDir), 0.0), reflectance);

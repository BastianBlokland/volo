...
bind_spec(1) const f32 s_terrainSize    = 50;
...
f32v3 heightmap_normal(const f32v2 uv, const f32 scale) {
  ...
  const f32 unitSize = 1.0 / textureSize(u_texHeightMap, 0).x * s_terrainSize;
...
void main() {
  ...
  const f32v3 normal  = heightmap_normal(in_texcoord, s_heightMapScale);

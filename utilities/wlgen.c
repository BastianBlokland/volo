...
// The project's XML lexer terminates attribute strings at newlines.
// Wayland protocol XML files occasionally have multi-line attribute values (e.g. summary="...\n  ...").
// This pass replaces newlines inside quoted attribute values with spaces.
...
static void wlgen_write_arg_type(WlGenContext* ctx, XmlDoc* doc, const XmlNode argNode) {
  ...
  if (string_eq(type, g_wlArgTypeInt) || string_eq(type, g_wlArgTypeFixed) ||
      string_eq(type, g_wlArgTypeFd)) {
...
static void wlgen_write_prolog(WlGenContext* ctx) {
  fmt_write(&ctx->out, "// Generated by '{}'", fmt_text(path_stem(g_pathExecutable)));
  dynarray_for_t(&ctx->protocols, WlGenProtocol, proto) {
    fmt_write(&ctx->out, " from '{}'", fmt_path(proto->path));
  }
  fmt_write(&ctx->out, ".\n");
}

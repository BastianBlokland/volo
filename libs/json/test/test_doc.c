...
spec(doc) {
  ...
  it("can retrieve the value of a string") {
    ...
    check_eq_u64(json_type(doc, str), JsonType_String);
  ...
  it("can retrieve the value of a number") {
    ...
    check_eq_u64(json_type(doc, val), JsonType_Number);
    check_eq_f64(json_number(doc, val), 42.1337, .1e-32);
  }

  it("can retrieve the value of a boolean") {
    const JsonVal val = json_add_bool(doc, true);

    check_eq_u64(json_type(doc, val), JsonType_Bool);
  ...
  it("can retrieve the type of null") {
    ...
    check_eq_u64(json_type(doc, val), JsonType_Null);
  }

  it("can store empty arrays") {
    const JsonVal val = json_add_array(doc);

    check_eq_u64(json_type(doc, val), JsonType_Array);
    check_eq_u64(json_elem_count(doc, val), 0);
  ...
  it("can store arrays with a single element") {
    ...
    check_eq_u64(json_parent(doc, elem), JsonParent_None);

    json_add_elem(doc, val, elem);

    check_eq_u64(json_parent(doc, elem), JsonParent_Array);

    check_eq_u64(json_type(doc, val), JsonType_Array);
    check_eq_u64(json_elem_count(doc, val), 1);
    check_eq_u64(json_elem_begin(doc, val), elem);
  ...
  it("can lookup array elements by index") {
    ...
    check_eq_u64(json_elem(doc, val, 0), elemVal1);
    check_eq_u64(json_elem(doc, val, 1), elemVal2);
    check_eq_u64(json_elem(doc, val, 2), elemVal3);
  ...
  it("can iterate array elements") {
    ...
    json_for_elems(doc, val, elem, {
      switch (i++) {
      case 0:
        check_eq_u64(json_type(doc, elem), JsonType_String);
        break;
      case 1:
        check_eq_u64(json_type(doc, elem), JsonType_Bool);
        break;
      case 2:
        check_eq_u64(json_type(doc, elem), JsonType_Null);
        break;
      default:
        check(false);
      }
    ...
    check_eq_u64(json_elem_count(doc, val), 3);
  }

  it("can store empty objects") {
    const JsonVal val = json_add_object(doc);

    check_eq_u64(json_type(doc, val), JsonType_Object);
    check_eq_u64(json_field_count(doc, val), 0);
  ...
  it("can store objects with a single field") {
    ...
    check_eq_u64(json_parent(doc, field), JsonParent_None);

    const bool res = json_add_field_str(doc, val, string_lit("a"), field);
    check(res);

    check_eq_u64(json_parent(doc, field), JsonParent_Object);

    check_eq_u64(json_type(doc, val), JsonType_Object);
    check_eq_u64(json_field_count(doc, val), 1);
    check_eq_string(json_field_begin(doc, val).name, string_lit("a"));
    check_eq_u64(json_field_begin(doc, val).value, field);
    check(
  ...
  it("can lookup object fields by name") {
    ...
    check_eq_u64(json_field(doc, val, string_lit("a")), fieldVal1);
    check_eq_u64(json_field(doc, val, string_lit("b")), fieldVal2);
    check_eq_u64(json_field(doc, val, string_lit("c")), fieldVal3);
  ...
  it("can iterate object fields") {
    ...
    json_for_fields(doc, val, itr, {
      ...
      case 0:
        ...
        check_eq_u64(json_type(doc, itr.value), JsonType_String);
        break;
      case 1:
        check_eq_string(itr.name, string_lit("b"));
        check_eq_u64(json_type(doc, itr.value), JsonType_Bool);
        break;
      case 2:
        check_eq_string(itr.name, string_lit("c"));
        check_eq_u64(json_type(doc, itr.value), JsonType_Null);
        break;
      default:
        check(false);
      }
    ...
    check_eq_u64(json_field_count(doc, val), 3);
  }
  ...
  it("can store complex structures") {
    ...
    json_for_fields(doc, root, rootItr, {
      ...
      case 0: {
        ...
        check_eq_u64(json_type(doc, json_field(doc, elem2, string_lit("a"))), JsonType_Null);
        check_eq_string(
            json_string(doc, json_field(doc, elem2, string_lit("b"))), string_lit("Hello"));
      } break;
      case 1:
        check_eq_f64(json_number(doc, rootItr.value), 42.0, .1e-32);
        break;
      case 2:
        check_eq_u64(

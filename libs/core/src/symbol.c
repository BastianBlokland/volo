...
struct sSymbolReg {
  ...
  SymbolAddrRel addrOffset; // Base address in the executable.
};
...
void symbol_reg_set_offset(SymbolReg* r, const SymbolAddrRel addrOffset) {
...
void symbol_stack_write(const SymbolStack* stack, DynString* out) {
  ...
  const SymbolAddrRel addrOffset = reg ? reg->addrOffset : 0;
  ...
  for (u32 frameIndex = 0; frameIndex != array_elems(stack->frames); ++frameIndex) {
    ...
    const SymbolAddrRel addrInExec = addr + addrOffset;

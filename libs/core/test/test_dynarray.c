...
spec(dynarray) {
  ...
  it("can create a new empty Dynamic-Array") {
    ...
    check_eq_u64(array.stride, sizeof(u64));
    check_eq_u64(array.size, 0);
  ...
  it("can be resized") {
    ...
    check_eq_u64(array.size, 1);

    dynarray_resize(&array, 33);
    check_eq_u64(array.size, 33);
  ...
  it("preserves content while resizing") {
    ...
    for (u32 i = 0; i != entries; ++i) {
      check_eq_u64(*dynarray_at_t(&array, i, u64), i);
    }
  ...
  it("updates the size while removing elements") {
    ...
    for (usize i = 0; i != array_elems(data); ++i) {
      ...
      check_eq_u64(array.size, array_elems(values) - data[i].removeCount);

      dynarray_for_t(&array, u64, val, {
        if (val_i < data[i].removeIdx)
          check_eq_u64(*val, values[val_i]);
        else
          check_eq_u64(*val, values[data[i].removeCount + val_i]);
      });
    }
    dynarray_destroy(&array);
  }
  ...
  it("moves the last element into the removed slot when using remove_unordered") {
    ...
    for (usize i = 0; i != array_elems(data); ++i) {
      ...
      check_eq_u64(array.size, data[i].initialSize - data[i].removeCount);

      dynarray_for_t(&array, u16, val, { check_eq_u64(*val, data[i].expected[val_i]); });
    }
    dynarray_destroy(&array);
  }
  ...
  it("updates the size when inserting elements") {
    ...
    for (usize i = 0; i != array_elems(data); ++i) {
      ...
      check_eq_u64(array.size, array_elems(values) + data[i].insertCount);

      dynarray_for_t(&array, u32, val, {
        if (val_i < data[i].insertIdx)
          check_eq_u64(*val, values[val_i]);
        else if (val_i < data[i].insertIdx + data[i].insertCount)
          check_eq_u64(*val, 0xBBBBBBBB);
        else
          check_eq_u64(*val, values[val_i - data[i].insertCount]);
      });
    }
    dynarray_destroy(&array);
  }
  ...
  it("can be sorted") {
    ...
    dynarray_for_t(&array, u32, val, { check_eq_u64(*val, expected[val_i]); });

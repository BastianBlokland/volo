...
static void test_file_verify_data(CheckTestContext* _testCtx, String input) {
  for (usize i = 0; i != input.size; ++i) {
    check_eq_u64(*string_at(input, i), i % 255);
  }
}
...
spec(file) {
  ...
  it("can read-back content that was written") {
    check_eq_u64(file_write_sync(tmpFile, string_lit("Hello World!")), FileResult_Success);
    check_eq_u64(file_seek_sync(tmpFile, 0), FileResult_Success);

    check_eq_u64(file_read_sync(tmpFile, &buffer), FileResult_Success);
  ...
  it("can read a file to the end") {
    ...
    check_eq_u64(file_write_sync(tmpFile, dynstring_view(&buffer)), FileResult_Success);
    check_eq_u64(file_seek_sync(tmpFile, 0), FileResult_Success);

    // Read the file to the end.
    dynstring_clear(&buffer);
    check_eq_u64(file_read_to_end_sync(tmpFile, &buffer), FileResult_Success);

    // Verify that all data was retrieved.
    check_eq_u64(buffer.size, testDataSize);
    test_file_verify_data(_testCtx, dynstring_view(&buffer));
  }

  it("can retrieve the file size") {
    check_eq_u64(file_stat_sync(tmpFile).size, 0);

    file_write_sync(tmpFile, string_lit("Hello World!"));
    check_eq_u64(file_stat_sync(tmpFile).size, 12);
  }

  it("can check the file-type of regular files") {
    check_eq_u64(file_stat_sync(tmpFile).type, FileType_Regular);
  }
  ...
  it("can check the file-type of directories") {
    ...
    if (workingDir) {
      check_eq_u64(file_stat_sync(workingDir).type, FileType_Directory);
  ...
  it("can read file contents through a memory map") {
    ...
    String mapping;
    check_eq_u64(file_map(tmpFile, &mapping), FileResult_Success);
  ...
  it("can read its own executable") {
    ...
    check_eq_u64(file_stat_sync(ownExecutable).type, FileType_Regular);

    check_eq_u64(file_read_sync(ownExecutable, &buffer), FileResult_Success);
    check(buffer.size > 0);

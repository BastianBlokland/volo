...
spec(bitset) {
  ...
  it("can test for non-set bits") {
    ...
    check_eq_u64(bitset_size(zero64), 64);
    check_eq_u64(bitset_count(zero64), 0);
    for (u32 i = 0; i != 64; ++i) {
  ...
  it("can test for set bits") {
    ...
    check_eq_u64(bitset_size(ones64), 64);
    check_eq_u64(bitset_count(ones64), 64);
    for (u32 i = 0; i != 64; ++i) {
  ...
  it("can find set bits") {
    ...
    check_eq_u64(bitset_size(bits), 64 * 32);

    // Check no bit is set.
    check_eq_u64(bitset_count(bits), 0);
    for (u32 i = 0; i != 64; ++i) {
    ...
    check_eq_u64(bitset_count(bits), 1);

    bitset_set(bits, 42);
    check(bitset_test(bits, 42));
    check_eq_u64(bitset_count(bits), 2);
  ...
  it("can count set bits") {
    ...
    check_eq_u64(bitset_count(bits), 5);
  }
  ...
  it("returns each bit for a mask with all bits set") {
    ...
    for (u32 i = 0; i != 64; ++i) {
      check_eq_u64(bitset_next(bits, i), i);
    }
  }

  it("can compute the index of a set bit") {
    const BitSet bits = bitset_from_var((u32){0b01000100010111000100010010010011});
    check_eq_u64(bitset_index(bits, 0), 0);
    check_eq_u64(bitset_index(bits, 1), 1);
    check_eq_u64(bitset_index(bits, 4), 2);
    check_eq_u64(bitset_index(bits, 7), 3);
    check_eq_u64(bitset_index(bits, 10), 4);
    check_eq_u64(bitset_index(bits, 14), 5);
    check_eq_u64(bitset_index(bits, 18), 6);
    check_eq_u64(bitset_index(bits, 19), 7);
    check_eq_u64(bitset_index(bits, 20), 8);
  }
  ...
  it("computes the same index as the iterations number while walking the set bits") {
    ...
    bitset_for(bits, setIdx, {
      check_eq_u64(bitset_index(bits, setIdx), i);
      ++i;
    });
  }
  ...
  it("can iterate all set bits") {
    ...
    check_eq_u64(bitset_count(bits), array_elems(indices));

    usize i = 0;
    bitset_for(bits, setIdx, { check_eq_u64(setIdx, indices[i++]); });
  }
  ...
  it("can bitwise 'or' two bitsets") {
    ...
    check_eq_u64(bitset_count(bits64), 32);
    bitset_or(bits64, unevenBits64);
    check_eq_u64(bitset_count(bits64), 64);
  ...
  it("can bitwise 'and' two bitsets") {
    ...
    check_eq_u64(bitset_count(evenBits64), 32);
    check_eq_u64(bitset_count(unevenBits64), 32);

    bitset_set(unevenBits64, 42);

    bitset_and(evenBits64, unevenBits64);
    check_eq_u64(bitset_count(evenBits64), 1);
    check_eq_u64(bitset_next(evenBits64, 0), 42);
  }
}

...
spec(scheduler) {
  ...
  it("can run a single-task job-graph") {
    ...
    JobId id = jobs_scheduler_run(jobGraph, g_allocPage);
  ...
  it("can run a job-graph multiple times") {
    ...
    DynArray jobIds = dynarray_create_t(g_allocHeap, JobId, g_numRuns);

    // Start the graph multiple times.
    for (usize i = 0; i != g_numRuns; ++i) {
      *dynarray_push_t(&jobIds, JobId) = jobs_scheduler_run(jobGraph, g_allocPage);
    }

...
int SYS_DECL main(const int argc, const char** argv) {
  ...
  const CliId optSymDump = cli_register_flag(app, '\0', string_lit("symbol-dump"), 0);
  cli_register_desc(app, optSymDump, string_lit("Dump a listing of all executable symbols."));
  ...
  if (cli_parse_provided(invoc, optSymDump)) {
    // TODO: Write symbols.
    goto exit;
  }

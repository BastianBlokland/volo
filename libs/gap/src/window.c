...
static void ecs_destruct_window_comp(void* data) {
  ...
  dynstring_destroy(&comp->textInput);
}
...
static void window_update(
  ...
  dynstring_append(&window->textInput, gap_pal_window_text_input(platform->pal, window->id));
...
String gap_window_text_input(const GapWindowComp* comp) { return dynstring_view(&comp->textInput); }

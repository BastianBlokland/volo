...
typedef enum eScriptOp {
  ...
  ScriptOp_Sin               = 35, // [d      ] (d      ) -> (d) Evaluate the sine function at 'd'.
  ScriptOp_Cos               = 36, // [d      ] (d      ) -> (d) Evaluate the cosine function at 'd'.
  ScriptOp_Tan               = 37, // [d      ] (d      ) -> (d) Evaluate the tangent function at 'd'.
  ScriptOp_Sqrt              = 38, // [d      ] (d      ) -> (d) Evaluate the square-root function at 'd'.
  ScriptOp_Pow               = 39, // [d,s    ] (d,s    ) -> (d) Raise the value in 'd' to the power of 's'.
  ScriptOp_Normalize         = 40, // [d      ] (d      ) -> (d) Normalize register 'd'.
  ScriptOp_Magnitude         = 41, // [d      ] (d      ) -> (d) Compute the magnitude of register 'd'.
  ScriptOp_Absolute          = 42, // [d      ] (d      ) -> (d) Normalize register 'd'.
  ScriptOp_VecX              = 43, // [d      ] (d      ) -> (d) Retrieve the x component of a vector in register 'd'.
  ScriptOp_VecY              = 44, // [d      ] (d      ) -> (d) Retrieve the y component of a vector in register 'd'.
  ScriptOp_VecZ              = 45, // [d      ] (d      ) -> (d) Retrieve the z component of a vector in register 'd'.
  ScriptOp_Vec3Compose       = 46, // [x,y,z  ] (x,y,z  ) -> (x) Compose a vector from 'x', 'y', 'z' and store in register 'x'.
  ScriptOp_QuatFromEuler     = 47, // [x,y,z  ] (x,y,z  ) -> (x) Compose a quaternion from 'x', 'y', 'z' angles and store in register 'x'.
  ScriptOp_QuatFromAngleAxis = 48, // [x,y    ] (x,y    ) -> (x) Compose a quaternion from angle 'x' and axis 'y' and store in register 'x'.
  ScriptOp_ColorR            = 49, // [d      ] (d      ) -> (d) Retrieve the r component of a color in register 'd'.
  ScriptOp_ColorG            = 50, // [d      ] (d      ) -> (d) Retrieve the g component of a color in register 'd'.
  ScriptOp_ColorB            = 51, // [d      ] (d      ) -> (d) Retrieve the b component of a color in register 'd'.
  ScriptOp_ColorA            = 52, // [d      ] (d      ) -> (d) Retrieve the a component of a color in register 'd'.
  ScriptOp_ColorCompose      = 53, // [x,y,z,w] (x,y,z,w) -> (x) Compose a color from 'x', 'y', 'z', 'w' and store in register 'x'.
  ScriptOp_ColorComposeHsv   = 54, // [x,y,z,w] (x,y,z,w) -> (x) Compose a hsv color from 'x', 'y', 'z', 'w' and store in register 'x'.
  ScriptOp_ColorFor          = 55, // [d      ] (d      ) -> (d) Compute a color for register 'd'.
  ScriptOp_Random            = 56, // [d      ] (       ) -> (d) Compute a random value (0 - 1) in register 'd'.
  ScriptOp_RandomSphere      = 57, // [d      ] (       ) -> (d) Compute a random vector on a unit sphere in register 'd'.
  ScriptOp_RandomCircleXZ    = 58, // [d      ] (       ) -> (d) Compute a random vector on a unit circle in register 'd'.
  ScriptOp_RandomBetween     = 59, // [x,y    ] (x,y    ) -> (x) Compute a random value between 'x' and 'y' and store in register 'x'.
  ScriptOp_RoundDown         = 60, // [d      ] (d      ) -> (d) Round register 'd' down.
  ScriptOp_RoundNearest      = 61, // [d      ] (d      ) -> (d) Round register 'd' to nearest.
  ScriptOp_RoundUp           = 62, // [d      ] (d      ) -> (d) Round register 'd' up.
  ScriptOp_Clamp             = 63, // [x,y,z  ] (x,y,z  ) -> (x) Clamp register 'x' between 'y' and 'z' and store in register 'x'.
  ScriptOp_Lerp              = 64, // [x,y,z  ] (x,y,z  ) -> (x) Compute a linearly interpolated value from 'x' to 'y' at time 'z' and store in register 'x'.
  ScriptOp_Min               = 65, // [x,y    ] (x,y    ) -> (x) Store the minimum value of 'x' and 'y' in register 'x'.
  ScriptOp_Max               = 66, // [x,y    ] (x,y    ) -> (x) Store the maximum value of 'x' and 'y' in register 'x'.
  ScriptOp_Perlin3           = 67, // [d      ] (       ) -> (d) Compute a 3d perlin noise in register 'd'.
} ScriptOp;

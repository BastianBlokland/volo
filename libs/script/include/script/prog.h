...
typedef enum eScriptOp {
  ...
  ScriptOp_Pow               = 37, // [d,s    ] (d,s    ) -> (d) Raise the value in 'd' to the power of 's'.
  ScriptOp_Normalize         = 38, // [d      ] (d      ) -> (d) Normalize register 'd'.
  ScriptOp_Magnitude         = 39, // [d      ] (d      ) -> (d) Compute the magnitude of register 'd'.
  ScriptOp_Absolute          = 40, // [d      ] (d      ) -> (d) Normalize register 'd'.
  ScriptOp_VecX              = 41, // [d      ] (d      ) -> (d) Retrieve the x component of a vector in register 'd'.
  ScriptOp_VecY              = 42, // [d      ] (d      ) -> (d) Retrieve the y component of a vector in register 'd'.
  ScriptOp_VecZ              = 43, // [d      ] (d      ) -> (d) Retrieve the z component of a vector in register 'd'.
  ScriptOp_Vec3Compose       = 44, // [x,y,z  ] (x,y,z  ) -> (x) Compose a vector from 'x', 'y', 'z' and store in register 'x'.
  ScriptOp_QuatFromEuler     = 45, // [x,y,z  ] (x,y,z  ) -> (x) Compose a quaternion from 'x', 'y', 'z' angles and store in register 'x'.
  ScriptOp_QuatFromAngleAxis = 46, // [x,y    ] (x,y    ) -> (x) Compose a quaternion from angle 'x' and axis 'y' and store in register 'x'.
  ScriptOp_ColorR            = 47, // [d      ] (d      ) -> (d) Retrieve the r component of a color in register 'd'.
  ScriptOp_ColorG            = 48, // [d      ] (d      ) -> (d) Retrieve the g component of a color in register 'd'.
  ScriptOp_ColorB            = 49, // [d      ] (d      ) -> (d) Retrieve the b component of a color in register 'd'.
  ScriptOp_ColorA            = 50, // [d      ] (d      ) -> (d) Retrieve the a component of a color in register 'd'.
  ScriptOp_ColorCompose      = 51, // [x,y,z,w] (x,y,z,w) -> (x) Compose a color from 'x', 'y', 'z', 'w' and store in register 'x'.
  ScriptOp_ColorComposeHsv   = 52, // [x,y,z,w] (x,y,z,w) -> (x) Compose a hsv color from 'x', 'y', 'z', 'w' and store in register 'x'.
  ScriptOp_ColorFor          = 53, // [d      ] (d      ) -> (d) Compute a color for register 'd'.
  ScriptOp_Random            = 54, // [d      ] (       ) -> (d) Compute a random value (0 - 1) in register 'd'.
  ScriptOp_RandomSphere      = 55, // [d      ] (       ) -> (d) Compute a random vector on a unit sphere in register 'd'.
  ScriptOp_RandomCircleXZ    = 56, // [d      ] (       ) -> (d) Compute a random vector on a unit circle in register 'd'.
  ScriptOp_RandomBetween     = 57, // [x,y    ] (x,y    ) -> (x) Compute a random value between 'x' and 'y' and store in register 'x'.
  ScriptOp_RoundDown         = 58, // [d      ] (d      ) -> (d) Round register 'd' down.
  ScriptOp_RoundNearest      = 59, // [d      ] (d      ) -> (d) Round register 'd' to nearest.
  ScriptOp_RoundUp           = 60, // [d      ] (d      ) -> (d) Round register 'd' up.
  ScriptOp_Clamp             = 61, // [x,y,z  ] (x,y,z  ) -> (x) Clamp register 'x' between 'y' and 'z' and store in register 'x'.
  ScriptOp_Lerp              = 62, // [x,y,z  ] (x,y,z  ) -> (x) Compute a linearly interpolated value from 'x' to 'y' at time 'z' and store in register 'x'.
  ScriptOp_Min               = 63, // [x,y    ] (x,y    ) -> (x) Store the minimum value of 'x' and 'y' in register 'x'.
  ScriptOp_Max               = 64, // [x,y    ] (x,y    ) -> (x) Store the maximum value of 'x' and 'y' in register 'x'.
  ScriptOp_Perlin3           = 65, // [d      ] (       ) -> (d) Compute a 3d perlin noise in register 'd'.
} ScriptOp;

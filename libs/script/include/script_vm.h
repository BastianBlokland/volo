...
typedef enum {
  ...
  ScriptOp_Vec3Compose       = 32, // [x,y,z  ] (x,y,z) -> (x) Compose a new vector from 'x', 'y', 'z' and store in register 'x'.
  ScriptOp_QuatFromEuler     = 33, // [x,y,z  ] (x,y,z) -> (x) Compose a new quaternion from 'x', 'y', 'z' angles and store in register 'x'.
  ScriptOp_QuatFromAngleAxis = 34, // [x,y    ] (x,y  ) -> (x) Compose a new quaternion from angle 'x' and axis 'y' and store in register 'x'.
} ScriptOp;

...
static void data_free_string(Allocator* alloc, const Mem data) {
...
static void data_free_struct(Allocator* alloc, const DataMeta meta, const Mem data) {
  ...
  for (usize i = 0; i != decl->val_struct.count; ++i) {
    ...
    data_free(alloc, field->meta, fieldMem);
  }
}

static void data_free_single(Allocator* alloc, const DataMeta meta, const Mem data) {
  ...
  case DataKind_String:
    data_free_string(alloc, data);
    return;
  case DataKind_Struct:
    data_free_struct(alloc, meta, data);
    return;
  case DataKind_Enum:
    return;
...
static void data_free_pointer(Allocator* alloc, const DataMeta meta, const Mem data) {
  ...
  data_free(alloc, targetMeta, targetMem);

  alloc_free(alloc, targetMem);
}

static void data_free_array(Allocator* alloc, const DataMeta meta, const Mem data) {
  const DataDecl*  decl  = data_decl(meta.type);
  ...
  for (usize i = 0; i != array->count; ++i) {
    ...
    data_free(alloc, elemMeta, elemMem);
  }

  alloc_free(alloc, mem_create(array->data, decl->size * array->count));
}

void data_free(Allocator* alloc, const DataMeta meta, const Mem data) {
  switch (meta.container) {
  case DataContainer_None:
    data_free_single(alloc, meta, data);
    return;
  case DataContainer_Pointer:
    data_free_pointer(alloc, meta, data);
    return;
  case DataContainer_Array:
    data_free_array(alloc, meta, data);
    return;
  }
  diag_crash();
}

...
ecs_system_define(LoadWeaponAssetSys) {
  ...
  for (EcsIterator* itr = ecs_view_itr(loadView); ecs_view_walk(itr);) {
    ...
    data_read_json(g_dataReg, src->data, g_allocHeap, g_assetWeaponMeta, mem_var(def), &readRes);
    ...
    data_destroy(g_dataReg, g_allocHeap, g_assetWeaponMeta, mem_var(def));
    if (buildErr) {
...
void asset_data_init_weapon(void) {
  ...
  g_assetWeaponMeta = data_meta_t(t_AssetWeaponMapDef);
}

...
static void ecs_destruct_terrain_comp(void* data) {
  ...
  data_destroy(
      g_dataReg, g_allocHeap, g_assetTerrainMeta, mem_create(comp, sizeof(AssetTerrainComp)));
}
...
void asset_data_init_terrain(void) {
  ...
  g_assetTerrainMeta = data_meta_t(t_AssetTerrainComp);
}

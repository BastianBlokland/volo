...
void asset_data_init_proctex(void) {
  ...
  g_assetTexProcMeta = data_meta_t(t_ProcTexDef);
}
...
void asset_load_tex_proc(
  ...
  data_read_json(g_dataReg, src->data, g_allocHeap, g_assetTexProcMeta, mem_var(def), &result);
...
Error:
  ...
  data_destroy(g_dataReg, g_allocHeap, g_assetTexProcMeta, mem_var(def));
  asset_repo_source_close(src);
}

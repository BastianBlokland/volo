...
static void ecs_destruct_level_comp(void* data) {
  ...
  data_destroy(g_dataReg, g_allocHeap, g_assetLevelMeta, mem_create(level, sizeof(AssetLevel)));
}
...
void asset_data_init_level(void) {
  ...
  g_assetLevelMeta = data_meta_t(t_AssetLevel);
}
...
void asset_load_level(
  ...
  data_read_json(g_dataReg, src->data, g_allocHeap, g_assetLevelMeta, mem_var(level), &readRes);
...
bool asset_level_save(AssetManagerComp* manager, const String id, const AssetLevel* level) {
  ...
  data_write_json(g_dataReg, &dataBuffer, g_assetLevelMeta, levelData, &jOpts);

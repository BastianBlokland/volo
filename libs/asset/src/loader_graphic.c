...
static void ecs_destruct_graphic_comp(void* data) {
  ...
  data_destroy(
      g_dataReg, g_allocHeap, g_assetGraphicMeta, mem_create(comp, sizeof(AssetGraphicComp)));
}
...
void asset_data_init_graphic(void) {
  ...
  g_assetGraphicMeta = data_meta_t(t_AssetGraphicComp);
}

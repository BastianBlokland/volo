...
spec(loader_graphic) {
  ...
  it("can load graphic assets") {
    ...
    check_require(graphic->shaders.values[0].specs.count == 1);
    check_eq_string(graphic->shaders.values[0].specs.values[0].name, string_lit("Test"));
    check_eq_int(graphic->shaders.values[0].specs.values[0].binding, 42);
    check_eq_float(graphic->shaders.values[0].specs.values[0].value, 1337.1337, 1e-8);

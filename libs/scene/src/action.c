...
ecs_view_define(ActionLightView) {
...
static void action_tell(ActionContext* ctx, const SceneActionTell* a) {
  if (!a->dst) {
    ...
    action_prop_set(propComp, a->dstProp, a->value, a->combinator);
    return;
  }

  if (ecs_view_maybe_jump(ctx->propertyItr, a->dst)) {
...
static void action_ask(ActionContext* ctx, const SceneActionAsk* a) {
  ScenePropertyComp* dstProp = null;
  if (a->dst) {
    if (ecs_view_maybe_jump(ctx->propertyItr, a->dst)) {
      dstProp = ecs_view_write_t(ctx->propertyItr, ScenePropertyComp);
    }
  } else {
    dstProp = ecs_view_write_t(ctx->globalItr, ScenePropertyComp);
  }

  const ScenePropertyComp* srcProp = null;
  if (a->src) {
    if (ecs_view_maybe_jump(ctx->propertyItr, a->src)) {
      srcProp = ecs_view_read_t(ctx->propertyItr, ScenePropertyComp);
    }
  } else {
    srcProp = ecs_view_read_t(ctx->globalItr, ScenePropertyComp);
  }

  if (dstProp && srcProp) {
    const ScriptVal val = scene_prop_load(srcProp, a->srcProp);
    action_prop_set(dstProp, a->dstProp, val, a->combinator);
  }
}
...
static void action_attach(ActionContext* ctx, const SceneActionAttach* a) {
  ...
  attach->offsetPos = a->offset;
  attach->offsetRot = geo_quat_ident;
}
...
static void action_update_light_param(ActionContext* ctx, const SceneActionUpdateLightParam* a) {
  if (ecs_view_maybe_jump(ctx->lightItr, a->entity)) {
    SceneLightAmbientComp* amb   = ecs_view_write_t(ctx->lightItr, SceneLightAmbientComp);
    SceneLightPointComp*   point = ecs_view_write_t(ctx->lightItr, SceneLightPointComp);
    SceneLightSpotComp*    spot  = ecs_view_write_t(ctx->lightItr, SceneLightSpotComp);
    SceneLightLineComp*    line  = ecs_view_write_t(ctx->lightItr, SceneLightLineComp);
    SceneLightDirComp*     dir   = ecs_view_write_t(ctx->lightItr, SceneLightDirComp);
    ...
    case SceneActionLightParam_Radiance:
      if (point) {
        point->radiance = a->value_color;
      }
      if (spot) {
        spot->radiance = a->value_color;
      }
      if (line) {
        line->radiance = a->value_color;
      }
      if (dir) {
        dir->radiance = a->value_color;
      }
      break;
    case SceneActionLightParam_Length:
      if (spot) {
        spot->length = a->value_f32;
      }
      if (line) {
        line->length = a->value_f32;
      }
      break;
    case SceneActionLightParam_Angle:
      if (spot) {
        spot->angle = a->value_f32;
      }
      break;
    }
  }
...
ecs_system_define(SceneActionUpdateSys) {
  ...
  ActionContext ctx = {
      ...
      .lightItr      = ecs_view_itr(ecs_world_view_t(world, ActionLightView)),
...
ecs_module_init(scene_action_module) {
  ...
  ecs_register_system(
      ...
      ecs_register_view(ActionLightView),

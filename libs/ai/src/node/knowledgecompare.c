...
static bool node_knowledge_greater(const AssetBehaviorKnowledgeCompare* comp, AiBlackboard* bb) {
  // TODO: Keys should be pre-hashed in the behavior asset.
  const StringHash            keyHash = string_hash(comp->key);
  const AssetKnowledgeSource* src     = &comp->value;
  ...
  switch (src->type) {
  case AssetKnowledgeSource_Number:
    return ai_blackboard_greater_f64(bb, keyHash, src->data_number.value);
  case AssetKnowledgeSource_Bool:
  case AssetKnowledgeSource_Vector:
    return false;
  case AssetKnowledgeSource_Time:
    return ai_blackboard_greater_time(bb, keyHash, node_src_time(&src->data_time, bb));
  case AssetKnowledgeSource_Knowledge: {
    // TODO: Keys should be pre-hashed in the behavior asset.
    const StringHash valueKeyHash = string_hash(src->data_knowledge.key);
    switch (ai_blackboard_type(bb, valueKeyHash)) {
    case AiBlackboardType_f64:
      return ai_blackboard_greater_f64(bb, keyHash, ai_blackboard_get_f64(bb, valueKeyHash));
    case AiBlackboardType_Time:
      return ai_blackboard_greater_time(bb, keyHash, ai_blackboard_get_time(bb, valueKeyHash));
    default:
      return false;
    }
  }
...
ai_node_knowledgecompare_eval(const AssetBehavior* behavior, AiBlackboard* bb, AiTracer* tracer) {
  ...
  case AssetKnowledgeComparison_Greater:
    result = node_knowledge_greater(&behavior->data_knowledgecompare, bb);
    break;
  }

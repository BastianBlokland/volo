...
spec(blackboard) {
  ...
  it("returns the fallback if knowledge is unset") {
  ...
  it("returns the stored knowledge") {
    ...
    check_eq_float(ai_blackboard_get_f64(bb, string_hash_lit("test"), 1337), 42, 1e-6f);
  }
  ...
  it("can store many knowledge keys") {
    ...
    for (u32 i = 0; i != keyCount; ++i) {
      const String key = fmt_write_scratch("test_{}", fmt_int(i));
      check_eq_float(ai_blackboard_get_f64(bb, string_hash(key), -1), i, 1e-6f);
    }
  }

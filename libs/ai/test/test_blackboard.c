...
spec(blackboard) {
  ...
  it("returns the stored knowledge") {
    ...
    check_eq_float(ai_value_get_f64(ai_blackboard_get(bb, string_hash_lit("test")), 0), 42, 1e-6f);
  }
  ...
  it("can store many knowledge keys") {
    ...
    for (u32 i = 0; i != KeyCount; ++i) {
      ...
      ai_blackboard_set(bb, string_hash(key), ai_value_f64(i));
    }
    for (u32 i = 0; i != KeyCount; ++i) {
      const String key = fmt_write_scratch("test_{}", fmt_int(i));
      check_eq_value(ai_blackboard_get(bb, string_hash(key)), ai_value_f64(i));
    }
  }
  ...
  it("can iterate blackboard keys") {
    ...
    for (AiBlackboardItr it = ai_blackboard_begin(bb); it.key; it = ai_blackboard_next(bb, it)) {
      const f64 val = ai_value_get_f64(ai_blackboard_get(bb, it.key), 0);
